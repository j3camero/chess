...
void TestPointBasicConstructor() {
  Point p(1, 4);
  ASSERT(p.rank == 1);
  ASSERT(p.file == 4);
}

void TestPointStringConstructorE2() {
  Point p("e2");
  ASSERT(p.rank == 6);
  ASSERT(p.file == 4);
}

void TestPointStringConstructorH6() {
  Point p("h6");
  ASSERT(p.rank == 2);
  ASSERT(p.file == 7);
}
...
void TestPointStringConstructorFail() {
...
void TestPointAdditionOperators() {
...
void TestPoint() {
  ...
  TestPointBasicConstructor();
  TestPointStringConstructorE2();
  TestPointStringConstructorH6();
  TestPointToString();
  TestPointIsOnBoard();
  TestPointStringConstructorFail();
  TestPointEqualityOperator();
  TestPointAdditionOperators();
}
